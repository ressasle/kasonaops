...
const normalizeAssetClass = (value: string | null) => {
  ...
  if (cleaned.includes("fund")) return "Other";
  ...
  try {
    ...
    if (mode === "single") {
      if (payload.asset_id) {
        ...
        const { outcome, update } = await enrichAsset(asset as PortfolioAssetRow, query, exchangeOverride, tickerOverride);
      ...
      if (existingAsset) {
        const { outcome, update } = await enrichAsset(existingAsset as PortfolioAssetRow, query, exchangeOverride);
        if (outcome.status === "error" || outcome.status === "skipped" || !update) {
          return NextResponse.json({ error: outcome.error ?? "Enrichment failed" }, { status: 400 });
        }
    ...
    if (mode === "portfolio") {
      ...
      for (const asset of assets) {
        const typedAsset = asset as PortfolioAssetRow;
    ...
    if (mode === "company") {
      ...
      for (const asset of assets) {
        const { outcome, update } = await enrichAsset(asset as PortfolioAssetRow, null, exchangeOverride);
        if (outcome.status === "updated" && update) {
          const { error: updateError } = await supabase
            .from("kasona_portfolio_assets")
            .update(update)
    ...
    if (mode === "customer") {
      ...
      const { data: profiles, error: profileError } = await supabase
        ...
        .select("portfolio_id")
        .eq("company_id", resolvedCustomerId);

      if (profileError) {
        return NextResponse.json({ error: profileError.message }, { status: 500 });
      }

      const portfolioIds = (profiles ?? []).map((profile) => profile.portfolio_id).filter(Boolean);
