...
export async function getStocksFromSearch(query: string) {
  const data = await callMcpTool<any>("get_stocks_from_search", {
    query,
    limit: 25,
    fmt: "json"
  });
  ...
  if (data && typeof data.result === "string") {
    try {
      return JSON.parse(data.result) as EodSearchResult[];
    } catch (e) {
      console.error("Failed to parse nested EODHD search result:", e);
      return [];
    }
  }

  // Fallback if it is already parsed or different structure
  return data as EodSearchResult[];
}
